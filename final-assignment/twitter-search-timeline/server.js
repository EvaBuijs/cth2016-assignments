...
function timeline_user(username, keyword, nbr_hits) {
  ...
  client.get('statuses/user_timeline', twitter_search_params, function(error, tweets, response) {
    ...
    for(tweet of tweets) {
      ...
      if(tweet.text.toLowerCase().search(keyword.toLowerCase()) > -1) {

        var r = {text: tweet.text}; // creating own data structure

         if(tweet.extended_entities && tweet.extended_entities.media) {

            r.images = []; // creating an array of images
...
io.on('connection', function(socket) {
  ...
  socket.on('search_twitter', function(msg) {
    ...
    timeline_user(msg.username, msg.keyword, msg.nbr_hits);
